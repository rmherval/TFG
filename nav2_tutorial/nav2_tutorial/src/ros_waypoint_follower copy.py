...
class MqttPointCommander(Node):
    def __init__(self):
        ...
        self.goal_sub = self.create_subscription(NavSatFix,'/goal_latlon',self.on_goal_msg,10)
    ...
    def on_mqtt_message(self, client, userdata, msg):
        try:
            ...
            x = payload['x']
            y = payload['y']
            self.get_logger().info(f"Punto MQTT recibido: x={x}, y={y}")
        except Exception as e:
            self.get_logger().error(f"Error al decodificar mensaje MQTT: {e}")
            return
